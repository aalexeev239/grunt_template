Шаблон для проектов, приносящий счастье и радость.
...
| `grunt ftp-deploy:build` и `grunt ftp-deploy:source`| кидают соответствующую ветку по **ftp** на хост. Требуется предварительная настройка.|
